...
def build_vocab():
    ...
    word_to_index = {unique_words[i]:i for i in range(word_count)}
    # 5.歌词文本用词表索引表示
    corpus_idx = []
    # 6.遍历每一行的分词结果
    ...
    # 7.返回结果:唯一词列表(5703个词)，词表{'想要':0，"有:1,'要大卖1:5702}，(去重后)词的数量，歌词文本用词表索引表示.
    return unique_words,word_to_index,word_count,corpus_idx
# 2. 数据预处理，构建数据集.
class LyricsDataset(torch.utils.data.Dataset): # 定义数据集类，继承torch.utils.data.Dataset
    # 1.初始化词索引，词个数等
...
class TextGenerator(nn.Module):
    ...
    def __init__(self,unique_word_count):
        ...
        self.ebd = nn.Embedding(unique_word_count,embedding_dim = 128)
        # 1.3.循环网络层：词向量的维度，隐藏层维度：256，网络层数：1
        self.rnn = nn.RNN(128,256,1)
        # 1.4.输出层（全连接层）：特征向量维度（和隐藏向量维度一致），词表中词的个数
        self.out = nn.Linear(256,unique_word_count)  # 词表中每个词的概率→选概率最大的词作为预测结果
    # 2.前向传播方法
...
def train():
    ...
    lyrics_dataloader = DataLoader(lyrics,5,True)
    # 5.定义损失函数
    ...
    for epoch in range(epochs):
        # 7.3.定义变量记录：本轮开始训练时间，迭代次数，训练总损失
        start,iter_num,total_loss = time.time(),0,0.0
        # 7.4.具体的本轮各批次训练动作
        # 遍历数据集，后台会调用LyricsDataset的__get()方法，获取到每个样本的数据和标签
...
def evaluate(start_word,sentence_length):
    ...
    # 6.定义列表，存放产生词的索引
    generate_sentence = [word_idx]  #开始词的索引，事列表的：第一个值
    # 7.遍历句子长度，获取到每一个词
    for i in range(sentence_length):
        # 7.1.模型预测
        output,hidden = model(torch.tensor([[word_idx]]),hidden)
        # 7.2.获取预测结果，argmax()从所有结果（5703个词的概率）中，找到最大值对应的索引
